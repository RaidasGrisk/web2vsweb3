...
const messages = computed(() => store.getters[store.getters['global/getBackend'] + '/getState'])

const getMessages = () => {
  console.log(store.getters['web2/getBackend'])
  store.dispatch('global/setLoading', true)
  console.log(store.getters['global/isLoading'])
  isLoading.value = true
  store.dispatch(store.getters['web2/getBackend'] + '/getMessages').then(() => {
    messages.value = store.getters[store.getters['web2/getBackend'] + '/getState']
...
<template>
  ...
  <div class="max-w-screen-1/2 h-56 py-5 mx-auto flex flex-col justify-center items-center">
    <div class="content-center">
      ...
      <div v-else class="shadow-md sm:rounded-lg">
        <table class="overflow-x-auto text-sm text-left text-gray-500 dark:text-gray-400">
          <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            ...
            <tbody>
              <tr v-for="(msg, index) in messages" :key="index" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
